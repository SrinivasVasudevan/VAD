...
DATA_DIR='C:\\Users\\srini\\conda prog\\VAD\\object_centric_VAD-master\\scripts\\data\\'
...
class GroundTruthLoader(object):
    ...
    SHANGHAITECH_LABEL_PATH = os.path.join(DATA_DIR, 'shanghaitech\\testing\\test_frame_mask')
    TOY_DATA = 'toydata'
    ...
    NAME_FRAMES_MAPPING = {
        AVENUE: os.path.join(DATA_DIR, 'avenue\\testing\\frames'),
        PED1: os.path.join(DATA_DIR, 'ped1\\testing\\frames'),
        PED2: os.path.join(DATA_DIR, 'ped2\\testing\\frames'),
        ENTRANCE: os.path.join(DATA_DIR, 'enter\\testing\\frames'),
        EXIT: os.path.join(DATA_DIR, 'exit\\testing\\frames')
    }
...
def dummy_eval(loss_file,reverse=True,smoothing=True):
    if not os.path.isdir(loss_file):
        loss_file_list = [loss_file]
    else:
        loss_file_list = os.listdir(loss_file)
    ...
    for sub_loss_file in loss_file_list:
        ...
        for i in range(num_videos):
            ...
            #_auc = metrics.auc(fpr, tpr)
            count=0
            #for x in np.array(gt[i],dtype=np.int8):
            #    count+=1
            #    print(count," :",x) 
            print(np.array(gt[i],dtype=np.int8))
            count=0
            for x in np.array(distance,dtype=np.float32):
                count+=1
                print(count," :",x) 
            print(np.array(distance,dtype=np.float32))
...
if __name__ == '__main__':
    ...
    evaluate_all("C:\\Users\\srini\\conda prog\\VAD\\object_centric_VAD-master\\scripts\\Score\\avenue.pkl")
    #dummy_eval("C:\\Users\\srini\\conda prog\\VAD\\object_centric_VAD-master\\scripts\\Score\\avenue_bn.pkl",True,True)
    # gt_loader = GroundTruthLoader()
